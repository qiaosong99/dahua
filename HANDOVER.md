...
## 五、验收状态（截至 2026-08-06）
- [x] 管理端连通性测试（页面实测“连接成功”）
- [x] 人脸下发/回收全流程（真机实测通过）
- [x] 访客记录刷脸时间列（真实数据匹配成功）
- [x] 二维码自动指向当前上网 IP + 刷新按钮（浏览器实测）
- [x] GBK 姓名修复（设备读回验证：新用户“编码验证”中文正常，旧 UTF-8 用户乱码印证根因）
- [x] 设备时间自动校准（连通性测试/人脸下发时同步）
- [ ] 设备屏幕确认开门记录中文姓名显示（需现场看设备）
- [ ] 手机扫码全流程体验新取景器（需真机）
- [ ] FRP 外网穿透（待 frps 服务端信息）
- 提示：首次部署需构建前端：`npm run build --prefix web`（产物输出到 server/public，不入库；不构建会提示“前端尚未构建”）
