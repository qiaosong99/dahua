...
public class ToolKits {
	...
	public static void StringToByteArray(String src, byte[] dst, int offset, int length) {
        if (src == null) return;
        byte[] bytes;
        try {
            bytes = src.getBytes();
        ...
        System.arraycopy(bytes, 0, dst, offset, len);
	 ...
	 * 字符串拷贝到char数组
	 */
	public static void StringToCharArray(String src, char[] dst) {
        for (int i = 0; i < dst.length; i++) {
            dst[i] = 0;
        }
        if (src == null) return;
        char[] chars = src.toCharArray();
        int len = Math.min(chars.length, dst.length);
        System.arraycopy(chars, 0, dst, 0, len);
    }
}
